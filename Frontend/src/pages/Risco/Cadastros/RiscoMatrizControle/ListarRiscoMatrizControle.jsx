...
const ListarRiscoMatrizControle = () => {
    ...
    // ----------------------------------------------------------------------------------------------
    const urlBackendMatrizControle = import.meta.env.VITE_API_URL_BACKEND + import.meta.env.VITE_API_URL_RISCO_MATRIZ_CONTROLE + "/datagrid";
    ...
    // ----------------------------------------------------------------------------------------------
    const { linhas, colunas, loading, error } = useFetchDatagrid(urlBackendMatrizControle);
    ...
    const handleIrPaginaCriarRegistro = () => {
        navegar(routes.risco_matriz_controle_criar);
    };
 
    const handleIrPaginaAtualizarRegistro = () => {
        const id = selectedId;
        navegar(routes.risco_matriz_controle_atualizar, { state: { id } });
    };

    const handleIrPaginaExcluirRegistro = () => {
        const id = selectedId
        navegar(routes.risco_matriz_controle_excluir, { state: { id } });
    };
        ...
        <MainLayout>
            <React.Fragment>
                ...
                <Actions breadcrumb="Início : Cadastros : Risco"
                    nomeSessao="Listagem das Matrizes de Controle" hasAddViewButton={false} hasFilter={false} />
...
export default ListarRiscoMatrizControle;