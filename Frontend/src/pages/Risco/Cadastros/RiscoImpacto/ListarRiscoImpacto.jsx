...
const ListarRiscoImpacto = () => {
    ...
    // ----------------------------------------------------------------------------------------------
    const urlBackendImpactoRisco = import.meta.env.VITE_API_URL_BACKEND + import.meta.env.VITE_API_URL_RISCO_IMPACTO_DATAGRID;
    ...
    // ----------------------------------------------------------------------------------------------
    const { linhas, colunas, loading, error } = useFetchDatagrid(urlBackendImpactoRisco);
    ...
    const handleIrPaginaCriarRegistro = () => {
        navegar(routes.risco_impacto_criar);
    };
 
    const handleIrPaginaAtualizarRegistro = () => {
        const id = selectedId;
        navegar(routes.risco_impacto_atualizar, { state: { id } });
    };

    const handleIrPaginaExcluirRegistro = () => {
        const id = selectedId
        navegar(routes.risco_impacto_excluir, { state: { id } });
    };
        ...
        <MainLayout>
            <React.Fragment>
                ...
                <Actions breadcrumb="Início : Cadastro : Risco"
                    nomeSessao="Listagem dos Impactos de Risco" hasAddViewButton={false} hasFilter={false} />
...
export default ListarRiscoImpacto;