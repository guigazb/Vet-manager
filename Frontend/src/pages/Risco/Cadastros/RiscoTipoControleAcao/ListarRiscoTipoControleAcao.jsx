...
const ListarRiscoTipoControleAcao = () => {
    ...
    // ----------------------------------------------------------------------------------------------
    const urlBackendTipoControleAcao = import.meta.env.VITE_API_URL_BACKEND + import.meta.env.VITE_API_URL_RISCO_TIPO_CONTROLE_ACAO_DATAGRID;
    // ----------------------------------------------------------------------------------------------
    // Variáveis de estado
    // ----------------------------------------------------------------------------------------------
    ...
    // ----------------------------------------------------------------------------------------------
    const { linhas, colunas, loading, error } = useFetchDatagrid(urlBackendTipoControleAcao);
    ...
    const handleIrPaginaCriarRegistro = () => {
        navegar(routes.risco_tipo_controle_acao_criar);
    };

    const handleIrPaginaAtualizarRegistro = () => {
        const id = selectedId;
        navegar(routes.risco_tipo_controle_acao_atualizar, { state: { id } });
    };

    const handleIrPaginaExcluirRegistro = () => {
        const id = selectedId
        navegar(routes.risco_tipo_controle_acao_excluir, { state: { id } });
    };
        ...
        <MainLayout>
            <React.Fragment>
                ...
                <Actions breadcrumb="Início : Cadastros : Risco"
                    nomeSessao="Listagem dos Tipos de Controle Ação" hasAddViewButton={false} hasFilter={false} />
...
export default ListarRiscoTipoControleAcao;