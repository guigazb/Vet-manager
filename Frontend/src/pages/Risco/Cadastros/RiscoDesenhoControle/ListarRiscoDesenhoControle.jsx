...
const ListarRiscoDesenhoControle = () => {
    ...
    // ----------------------------------------------------------------------------------------------      
    const urlBackendDesenhoControleRisco = import.meta.env.VITE_API_URL_BACKEND + import.meta.env.VITE_API_URL_RISCO_DESENHO_CONTROLE_DATAGRID;
    // ----------------------------------------------------------------------------------------------
    // Variáveis de estado
    // ----------------------------------------------------------------------------------------------
    ...
    // ----------------------------------------------------------------------------------------------
    const { linhas, colunas, loading, error } = useFetchDatagrid(urlBackendDesenhoControleRisco);
    ...
    const handleIrPaginaCriarRegistro = () => {
        navegar(routes.risco_desenho_controle_criar);
    };

    const handleIrPaginaAtualizarRegistro = () => {
        const id = selectedId;
        navegar(routes.risco_desenho_controle_atualizar, { state: { id } });
    };

    const handleIrPaginaExcluirRegistro = () => {
        const id = selectedId
        navegar(routes.risco_desenho_controle_excluir, { state: { id } });
    };
    ...
    return (
        <MainLayout>
            <React.Fragment>
                ...
                <Actions breadcrumb="Início : Cadastros : Desenho Controle"
                    nomeSessao="Listagem dos Desenhos de controle" hasAddViewButton={false} hasFilter={false} />
...
export default ListarRiscoDesenhoControle;