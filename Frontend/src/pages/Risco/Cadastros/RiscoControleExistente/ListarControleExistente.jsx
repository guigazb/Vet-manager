...
const ListarControleExistente = () => {
    ...
    const urlBackendControlesExistentes = import.meta.env.VITE_API_URL_BACKEND + import.meta.env.VITE_API_URL_RISCO_CONTROLES_EXISTENTES + "/" + localExecucaoId + "/datagrid/" + riscoId;
    ...
    // ----------------------------------------------------------------------------------------------
    const { linhas, colunas, loading, error } = useFetchDatagrid(urlBackendControlesExistentes);
    ...
    const handleIrPaginaCriarRegistro = () => {
        navegar(routes.risco_controle_existente_criar, { state: { riscoId, unidadeFuncional } });
    };

    const handleIrPaginaAtualizarRegistro = () => {
        const id = selectedId;
        navegar(routes.risco_controle_existente_atualizar, { state: { riscoId, unidadeFuncional, id } });
    };

    const handleIrPaginaExcluirRegistro = () => {
        const id = selectedId
        navegar(routes.risco_controle_existente_excluir, { state: { riscoId, unidadeFuncional, id } });
    };
    ...
    return (
        <MainLayout>
            <React.Fragment>
                ...
                <Actions breadcrumb="Início : Cadastro : Riscos" 
                nomeSessao={"Listar Controles Existentes do Risco: " + (riscoDescricaoTruncada ? riscoDescricaoTruncada : "")} 
...
export default ListarControleExistente