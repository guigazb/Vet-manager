...
const ListarRiscoRespostaEvento = () => {
    ...
    // ----------------------------------------------------------------------------------------------
    const urlBackendRespostaEventoRisco = import.meta.env.VITE_API_URL_BACKEND + import.meta.env.VITE_API_URL_RISCO_RESPOSTA_EVENTO_RISCO_DATAGRID;
    // ----------------------------------------------------------------------------------------------
    // Variáveis de estado
    // ----------------------------------------------------------------------------------------------
    ...
    // ----------------------------------------------------------------------------------------------
    const { linhas, colunas, loading, error } = useFetchDatagrid(urlBackendRespostaEventoRisco);
    ...
    const handleIrPaginaCriarRegistro = () => {
        navegar(routes.risco_resposta_risco_criar);
    };

    const handleIrPaginaAtualizarRegistro = () => {
        const id = selectedId;
        navegar(routes.risco_resposta_risco_atualizar, { state: { id } });
    };

    const handleIrPaginaExcluirRegistro = () => {
        const id = selectedId
        navegar(routes.risco_resposta_risco_excluir, { state: { id } });
    };
        ...
        <MainLayout>
            <React.Fragment>
                ...
                <Actions breadcrumb="Início : Cadastros : Risco"
                    nomeSessao="Listagem das Respostas para Eventos de Risco" hasAddViewButton={false} hasFilter={false} />
...
export default ListarRiscoRespostaEvento;