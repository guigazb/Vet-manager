...
const ListarRiscoProbabilidade = () => {
    ...
    // ----------------------------------------------------------------------------------------------
    const urlBackendProbabilidadeRisco = import.meta.env.VITE_API_URL_BACKEND + import.meta.env.VITE_API_URL_RISCO_PROBABILIDADE_DATAGRID;
    ...
    const handleIrPaginaCriarRegistro = () => {
        navegar(routes.risco_probabilidade_criar);
    };

    const handleIrPaginaAtualizarRegistro = () => {
        const id = selectedId;
        navegar(routes.risco_probabilidade_atualizar, { state: { id } });
    };

    const handleIrPaginaExcluirRegistro = () => {
        const id = selectedId
        navegar(routes.risco_probabilidade_excluir, { state: { id } });
    };
        ...
        <MainLayout>
            <React.Fragment>
                ...
                <Actions breadcrumb="Início : Cadastro : Risco"
                    nomeSessao="Listagem das Probabilidades de Risco" hasAddViewButton={false} hasFilter={false} />
...
export default ListarRiscoProbabilidade;