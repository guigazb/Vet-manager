...
const ListarRiscoCausa = () => {
    ...
    // ----------------------------------------------------------------------------------------------  
    const urlBackendRiscoCausa = import.meta.env.VITE_API_URL_BACKEND + import.meta.env.VITE_API_URL_RISCO_CAUSA_DATAGRID + "/" + riscoId;

    // ----------------------------------------------------------------------------------------------
    // Busca informações via hook
    // ----------------------------------------------------------------------------------------------
    const { linhas, colunas, loading, error } = useFetchDatagrid(urlBackendRiscoCausa);
    ...
    const handleIrPaginaCriarRegistro = () => {
        navigate(routes.risco_causa_criar, { state: { riscoId, unidadeFuncional } });
    };

    const handleIrPaginaAtualizarRegistro = () => {
        const id = selectedId;
        navigate(routes.risco_causa_atualizar, { state: { riscoId, unidadeFuncional, id } });
    };

    const handleIrPaginaExcluirRegistro = () => {
        const id = selectedId;
        navigate(routes.risco_causa_excluir, { state: { riscoId, unidadeFuncional, id } });
    };

    const handleIrParaAtualizarRisco = () => {
        const id = riscoId;
        navigate(routes.risco_atualizar, { state: { id, unidadeFuncional } });
    };
    ...
    return (
        <MainLayout>
            <React.Fragment>
                <Actions
                    breadcrumb="Início : Risco : Causas"
                    nomeSessao={`Listar Causas do Risco: ` + (riscoDescricaoTruncada ? riscoDescricaoTruncada : "")}
...
export default ListarRiscoCausa;