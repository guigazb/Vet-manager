...
const ListarPermissaoGrupo = () => {
    ...
    // ----------------------------------------------------------------------------------------------  
    const urlBackendGrupoPermissão = import.meta.env.VITE_API_URL_BACKEND + import.meta.env.VITE_API_URL_PERMISSAO_GRUPO_DATAGRID;
    // ----------------------------------------------------------------------------------------------
    // Variáveis de estado
    // ----------------------------------------------------------------------------------------------
    ...
    // ----------------------------------------------------------------------------------------------
    const { linhas, colunas, loading, error } = useFetchDatagrid(urlBackendGrupoPermissão);
    ...
    const handleIrPaginaCriarRegistro = () => {
        navegar(routes.diversos_permissao_grupo_criar);
    };

    const handleIrPaginaAtualizarRegistro = () => {
        const id = selectedId;
        navegar(routes.diversos_permissao_grupo_atualizar, { state: { id } });
    };

    const handleIrPaginaExcluirRegistro = () => {
        const id = selectedId
        navegar(routes.diversos_permissao_grupo_excluir, { state: { id } });
    };
        ...
        <MainLayout>
            <React.Fragment>
                ...
                <Actions breadcrumb="Início : Diversos"
                    nomeSessao="Listagem de Grupo de Permissão" hasAddViewButton={false} hasFilter={false} />
...
export default ListarPermissaoGrupo;