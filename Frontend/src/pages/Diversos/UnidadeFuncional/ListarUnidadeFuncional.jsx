...
const ListarUnidadeFuncional = () => {
    ...
    // ----------------------------------------------------------------------------------------------  
    const urlBackendUnidadeFuncional = import.meta.env.VITE_API_URL_BACKEND + import.meta.env.VITE_API_URL_UNIDADE_FUNCIONAL_DATAGRID;
    // ----------------------------------------------------------------------------------------------
    // Variáveis de estado
    // ----------------------------------------------------------------------------------------------
    ...
    // ----------------------------------------------------------------------------------------------
    const { linhas, colunas, loading, error } = useFetchDatagrid(urlBackendUnidadeFuncional);
    ...
    const handleIrPaginaCriarRegistro = () => {
        navegar(routes.diversos_unidade_funcional_criar);
    };

    const handleIrPaginaAtualizarRegistro = () => {
        const id = selectedId;
        navegar(routes.diversos_unidade_funcional_atualizar, { state: { id } });
    };

    const handleIrPaginaExcluirRegistro = () => {
        const id = selectedId
        navegar(routes.diversos_unidade_funcional_excluir, { state: { id } });
    };
        ...
        <MainLayout>
            <React.Fragment>
                ...
                <Actions breadcrumb="Início : Diversos"
                    nomeSessao="Listagem das Unidades Funcionais" hasAddViewButton={false} hasFilter={false} />
...
export default ListarUnidadeFuncional;