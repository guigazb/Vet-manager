...
function ListarTipoNormativo() {
    ...
    // ----------------------------------------------------------------------------------------------  
    const urlBackendTipoNormativo = import.meta.env.VITE_API_URL_BACKEND + import.meta.env.VITE_API_URL_DOCUMENTO_NORMATIVO_TIPO_DATAGRID;
    ...
    // ----------------------------------------------------------------------------------------------
    const { linhas, colunas, loading, error } = useFetchDatagrid(urlBackendTipoNormativo);
    ...
    const handleIrPaginaCriarRegistro = () => {
        navegar(routes.diversos_tipo_normativo_criar);
    };

    const handleIrPaginaAtualizarRegistro = () => {
        const id = selectedId;
        navegar(routes.diversos_tipo_normativo_atualizar, { state: { id } });
    };

    const handleIrPaginaExcluirRegistro = () => {
        const id = selectedId
        navegar(routes.diversos_tipo_normativo_excluir, { state: { id } });
    };
        ...
        <MainLayout>
            <React.Fragment>
                ...
                <Actions breadcrumb="Início : Diversos"
                    nomeSessao="Listagem dos tipos de Normativos" hasAddViewButton={false} hasFilter={false} />
...
export default ListarTipoNormativo;