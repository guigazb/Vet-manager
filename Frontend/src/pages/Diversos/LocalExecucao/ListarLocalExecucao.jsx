...
const ListarLocalExecucao = () => {
    ...
    // ----------------------------------------------------------------------------------------------  
    const urlBackendLocalExecucao = import.meta.env.VITE_API_URL_BACKEND + import.meta.env.VITE_API_URL_LOCAL_EXECUCAO_DATAGRID;
    ...
    // ----------------------------------------------------------------------------------------------
    const { linhas, colunas, loading, error } = useFetchDatagrid(urlBackendLocalExecucao);
    ...
    const handleIrPaginaCriarRegistro = () => {
        navegar(routes.diversos_local_execucao_criar);
    };

    const handleIrPaginaAtualizarRegistro = () => {
        const id = selectedId;
        navegar(routes.diversos_local_execucao_atualizar, { state: { id } });
    };

    const handleIrPaginaExcluirRegistro = () => {
        const id = selectedId
        navegar(routes.diversos_local_execucao_excluir, { state: { id } });
    };
        ...
        <MainLayout>
            <React.Fragment>
                ...
                <Actions breadcrumb="Início : Diversos"
                    nomeSessao="Listagem de Locais de Execução" hasAddViewButton={false} hasFilter={false} />
...
export default ListarLocalExecucao;