...
const ListarPermissao = () => {
    ...
    // ----------------------------------------------------------------------------------------------  
    const urlBackendPermissao = import.meta.env.VITE_API_URL_BACKEND + import.meta.env.VITE_API_URL_PERMISSAO_DATAGRID;
    // ----------------------------------------------------------------------------------------------
    // Variáveis de estado
    // ----------------------------------------------------------------------------------------------
    ...
    // ----------------------------------------------------------------------------------------------
    const { linhas, colunas, loading, error } = useFetchDatagrid(urlBackendPermissao);
    ...
    const handleIrPaginaCriarRegistro = () => {
        navegar(routes.diversos_permissao_criar);
    };

    const handleIrPaginaAtualizarRegistro = () => {
        const id = selectedId;
        navegar(routes.diversos_permissao_atualizar, { state: { id } });
    };

    const handleIrPaginaExcluirRegistro = () => {
        const id = selectedId
        navegar(routes.diversos_permissao_excluir, { state: { id } });
    };
        ...
        <MainLayout>
            <React.Fragment>
                ...
                <Actions breadcrumb="Início : Diversos"
                    nomeSessao="Listagem de Permissões" hasAddViewButton={false} hasFilter={false} />
                ...
                <Box sx={{ height: 400, width: '100%' }}>
                    <DataGrid
                        ...
                        showToolbar // consertar - não exibe o toolbar
                    />
                </Box>
...
export default ListarPermissao;